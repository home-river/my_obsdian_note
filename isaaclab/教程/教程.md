...
##
# 预定义资源
### 导入预设的 cartpole 机器人配置  # isort:skip
...
这个脚本演示了如何为倒立摆平衡任务运行强化学习（RL）环境。

.. code-block:: bash

    ./isaaclab.sh -p scripts/tutorials/03_envs/run_cartpole_rl_env.py --num_envs 32
    # 上述命令展示了如何通过 isaaclab.sh 启动脚本，并指定并行环境数量为32个。
"""

"""Launch Isaac Sim Simulator first."""
# 提示：在运行此脚本之前，需要先启动 Isaac Sim 仿真器。
# 通常，这个脚本会通过 AppLauncher 来启动 Isaac Sim。

import argparse # 导入 argparse 模块，用于解析命令行参数。
# 从 isaaclab.app 模块导入 AppLauncher 类，用于启动和管理 Isaac Sim 应用程序。
from isaaclab.app import AppLauncher 
...
# 将 AppLauncher 相关的命令行参数（如 --headless, --device 等）添加到当前的解析器中
AppLauncher.add_app_launcher_args(parser)
# parse the arguments
# 解析命令行传入的参数
args_cli = parser.parse_args()
...
# 使用解析后的参数初始化 AppLauncher
app_launcher = AppLauncher(args_cli)
# 获取 Isaac Sim 应用程序实例
simulation_app = app_launcher.app

"""Rest everything follows."""
# 提示：接下来的代码将在 Isaac Sim 应用程序成功启动后执行。

 # 导入 PyTorch 库，用于张量操作和神经网络。
import torch
# 从 isaaclab.envs 模块导入 ManagerBasedRLEnv 类，这是 Isaac Lab 中基于管理器的强化学习环境基类。
from isaaclab.envs import ManagerBasedRLEnv 
# 从特定任务路径导入 CartpoleEnvCfg 配置类，该类定义了倒立摆环境的各种参数。
...
def main():
    ...
    # 根据命令行参数更新配置中的环境数量
    env_cfg.scene.num_envs = args_cli.num_envs
    # 根据命令行参数更新配置中的仿真设备（例如 "cuda:0" 或 "cpu"）
    env_cfg.sim.device = args_cli.device # args_cli.device 通常由 AppLauncher.add_app_launcher_args 添加并解析
    ...
    # simulate physics
    # 模拟物理过程
    count = 0 # 初始化一个计数器，用于控制重置频率
    # 当 Isaac Sim 仿真应用仍在运行时，循环执行
    while simulation_app.is_running():
        # 使用 torch.inference_mode() 上下文管理器，以禁用梯度计算，提高推断效率
        with torch.inference_mode():
            # reset
            # 如果计数器是300的倍数（即每300个仿真步）
            if count % 300 == 0:
                count = 0 # 重置计数器
                env.reset() # 调用环境的 reset 方法，重置所有并行环境到初始状态
                print("-" * 80) # 打印分隔线
                print("[INFO]: Resetting environment...") # 打印重置信息
            # sample random actions
            # 生成与环境动作管理器中定义的动作形状和设备相同的随机动作张量
            # env.action_manager.action 是一个包含了当前动作的张量
            joint_efforts = torch.randn_like(env.action_manager.action)
            # step the environment
            # 执行环境的一步，传入随机生成的动作
            # 返回观测(obs), 奖励(rew), 终止标志(terminated), 截断标志(truncated), 以及额外信息(info)
            obs, rew, terminated, truncated, info = env.step(joint_efforts)
            # print current orientation of pole
            # 打印第一个环境 (Env 0) 中倒立摆杆子的当前关节位置（通常是角度）
            # obs["policy"] 是为策略网络准备的观测数据组
            # [0] 选择第一个环境的数据
            # [1] 假设倒立摆杆子的角度是观测向量中的第二个元素 (索引为1)
    ...
    # 当仿真循环结束后（例如用户关闭了仿真窗口），关闭环境，释放资源
    env.close()
...
场景定义 (`CartpoleSceneCfg`)使用交互式场景，方便多次创建。

配置动作空间 `ActionsCfg`、观测空间 `ObservationsCfg`、事件/重置逻辑 `EventCfg`、奖励函数 `RewardsCfg`、终止条件 `TerminationsCfg`。

最后进行环境总配置 `CartpoleEnvCfg`。最后在另一个运行的文件中进行实例化，实现仿真的交互就行。

---------
# 创建直接工作流RL环境

除了推荐使用配置类（configuration classes）实现模块化环境的 **envs.ManagerBasedRLEnv** 类外，**DirectRLEnv** 类为环境脚本化开发提供了更直接的控制方式。

与通过Manager类定义奖励和观测的范式不同，直接工作流（direct workflow）将完整的奖励函数和观测逻辑直接实现在任务脚本中。这种方式具有以下优势：

- 支持更灵活的方法实现（如调用PyTorch JIT特性）
    
- 提供更低抽象度的框架结构，便于代码定位与调试
    

本教程将通过直接工作流实现倒立摆环境配置，创建"保持竖直平衡"任务。我们将学习如何通过以下函数实现任务规范：
...
### 直接工作流代码
[创建直接工作流RL环境 — Isaac Lab 文档](https://docs.robotsfan.com/isaaclab/source/tutorials/03_envs/create_direct_rl_env.html)
```python
...
class CartpoleEnvCfg(DirectRLEnvCfg):
    # 决策频率与时长
    decimation = 2                     # 一步决策两步仿真
    episode_length_s = 5.0            # 单回合最大时长（秒）

    # 动作与观测维度
    action_scale = 100.0              # 动作缩放因子（N）
    action_space = 1                  # 动作空间维度
    observation_space = 4             # 观测空间维度
    state_space = 0                   # 不额外使用状态空间
    ...
    scene: InteractiveSceneCfg = InteractiveSceneCfg(
        ...
        replicate_physics=True #允许物理模块复用，减小开销
    )

    # 重置范围
    max_cart_pos = 3.0                # 小车最大允许位置（米）
    initial_pole_angle_range = [-0.25, 0.25]  # 杆角初始随机范围（π弧度）
...
class CartpoleEnv(DirectRLEnv):
    cfg: CartpoleEnvCfg #声明cfg是上面的CartpoleEnvCfg

    def __init__(self, cfg: CartpoleEnvCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)
        # 查找关节索引并缓存关节状态张量
        self._cart_dof_idx, _ = 
        self.cartpole.find_joints(self.cfg.cart_dof_name)
        
        self._pole_dof_idx, _ = 
        self.cartpole.find_joints(self.cfg.pole_dof_name)
    ...
    def _get_observations(self) -> dict:
        # 返回 [杆角度, 杆角速, 车位置, 车速]
        p_pos = self.joint_pos[:, self._pole_dof_idx[0]].unsqueeze(1)
        p_vel = self.joint_vel[:, self._pole_dof_idx[0]].unsqueeze(1)
        c_pos = self.joint_pos[:, self._cart_dof_idx[0]].unsqueeze(1)
        c_vel = self.joint_vel[:, self._cart_dof_idx[0]].unsqueeze(1)
        return {"policy": torch.cat((p_pos, p_vel, c_pos, c_vel), dim=-1)}
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        oob_cart = torch.abs(self.joint_pos[:, self._cart_dof_idx]) > self.cfg.max_cart_pos
        oob_pole = torch.abs(self.joint_pos[:, self._pole_dof_idx]) > math.pi / 2
        terminated = torch.any(oob_cart | oob_pole, dim=1)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        jp = self.cartpole.data.default_joint_pos[env_ids].clone()
        jp[:, self._pole_dof_idx] += sample_uniform(
            self.cfg.initial_pole_angle_range[0] * math.pi,
            self.cfg.initial_pole_angle_range[1] * math.pi,
            jp[:, self._pole_dof_idx].shape,
            jp.device,
        )
        jv = self.cartpole.data.default_joint_vel[env_ids].clone()
        root = self.cartpole.data.default_root_state[env_ids].clone()
        root[:, :3] += self.scene.env_origins[env_ids]
        # 写回仿真
        self.joint_pos[env_ids] = jp
        self.joint_vel[env_ids] = jv
        self.cartpole.write_root_pose_to_sim(root[:, :7], env_ids)
        self.cartpole.write_root_velocity_to_sim(root[:, 7:], env_ids)
        self.cartpole.write_joint_state_to_sim(jp, jv, None, env_ids)
...
def compute_rewards(
    ...
    # 存活奖励 + 失败惩罚 + 位置/速度惩罚
    r_alive = rew_scale_alive * (1.0 - reset_terminated.float())
    r_term = rew_scale_terminated * reset_terminated.float()
    r_pp = rew_scale_pole_pos * torch.square(pole_pos)
    r_cv = rew_scale_cart_vel * torch.abs(cart_vel)
    r_pv = rew_scale_pole_vel * torch.abs(pole_vel)
    return r_alive + r_term + r_pp + r_cv + r_pv

```


|特性／方式|直接 RL（DirectRLEnv）|基于管理器（EnvManager/RL App）|
|---|---|---|
|**核心 API**|继承自 `DirectRLEnv`，重写 `_setup_scene`, `_pre_physics_step`, `_apply_action`,  <br>`_get_observations`, `_get_rewards`, `_get_dones`, `_reset_idx` 等钩子。|使用 `RLEnvManager`、`Task`、`AgentManager`、`RLTrainer`／`RLApp` 等高层组件。  <br>通过配置文件或 Python 脚本组装环境、策略、训练循环。|
|**并行化 & 性能**|依赖 PhysX 的“环境克隆”（`clone_environments` + `replicate_physics`），在 GPU 上一次性创建 N 个仿真副本。  <br>所有计算（观测、奖励、done 判定、重置）都向量化在一个张量上并行完成，性能最优。|EnvManager 在内部也能批量步进多个环境，但它更像多个 “Gym 环境” 的集合，每个环境有自己的状态机。  <br>默认并行度较低，需要显式配置并行 workers，容易受 Python GIL/IPC 开销影响。|
|**可定制性 & 代码量**|你需要自己写全套环境逻辑：如何加载场景、施加动作、计算奖励、判 dones、重置；代码量较多，但精细可控、性能最大化。|大部分流程由管理器/框架接管，你只需实现“任务配置”（场景、机器人、观察／动作映射、奖励构造）和  <br>`Agent` 的接口，训练循环、日志、检查点都开箱即用。|
|**扩展性 & 生态**|直接接触底层 API，想加哪些自定义功能或整合第三方库都很灵活；但要自己解决所有“工程”问题。|生态成熟：配套 tensorboard、wandb、RL 监控面板、分布式训练、动态图形界面都已集成；对新手友好。|
|**典型场景**|“追求极致吞吐量”或 “需要高度定制” 时；  <br>大型算力集群、离线快速收集样本。|“快速上手”、“小规模实验”或 “需要多种监控/可视化” 时；  <br>想少写样板代码，专注算法开发。|
...
from isaaclab.app import AppLauncher  # 从 isaaclab.app 导入应用启动器

# 添加 argparse 参数
parser = argparse.ArgumentParser(description="Random agent for Isaac Lab environments.")  # 创建解析器，描述随机智能体脚本
parser.add_argument(
    "--disable_fabric", action="store_true", default=False, help="Disable fabric and use USD I/O operations."
)  # 可选参数：禁用 Fabric
parser.add_argument("--num_envs", type=int, default=None, help="Number of environments to simulate.")  # 环境数量
parser.add_argument("--task", type=str, default=None, help="Name of the task.")  # 任务名称
# 将 AppLauncher 的命令行参数追加到解析器中
AppLauncher.add_app_launcher_args(parser)
# 解析命令行参数
args_cli = parser.parse_args()

# 启动 Omniverse 应用
app_launcher = AppLauncher(args_cli)  # 创建 AppLauncher 实例
simulation_app = app_launcher.app  # 获取仿真应用对象
...
import isaaclab_tasks  # noqa: F401  # 导入任务注册模块
from isaaclab_tasks.utils import parse_env_cfg  # 导入环境配置解析工具
...
def main():
    ...
    # 创建环境
    env = gym.make(args_cli.task, cfg=env_cfg)  # 使用 Gym 创建环境

    # 打印环境信息（向量化环境）
    print(f"[INFO]: Gym observation space: {env.observation_space}")  # 输出观测空间
    print(f"[INFO]: Gym action space: {env.action_space}")  # 输出动作空间
    # 重置环境
    env.reset()  # 初始化环境状态
    # 运行仿真
    while simulation_app.is_running():  # 当仿真应用在运行时
        # 以推理模式运行，无梯度计算
        with torch.inference_mode():
            # 从 -1 到 1 随机采样动作
...
if __name__ == "__main__":
    ...
    main()
    # 关闭仿真应用
    simulation_app.close()  # 关闭 Omniverse 仿真应用
...
以下摘自 Isaac Lab 文档中的“Manager-Based Environments”部分，对 Cartpole 环境的注册调用：

```python
import gymnasium as gym

from . import agents  # 定义不同 RL 库的配置入口

# 将环境注册到 Gym
gym.register(
    id="Isaac-Cartpole-v0",                              # 环境唯一标识
    entry_point="isaaclab.envs:ManagerBasedRLEnv",       # 环境实现的模块与类
    disable_env_checker=True,                            # 关闭 Gym 自带的环境合法性检查
    kwargs={                                         # 传给环境构造函数的默认参数
...
> - **`id`**：环境名称，需在全局唯一（常见格式 `<前缀>-<任务>-<版本>`）。
>     
> - **`entry_point`**：实际环境类的导入路径，格式为 `"<模块路径>:<类名>"`。
>     
> - **`kwargs`**：默认传给构造函数的参数，比如环境配置入口点（`env_cfg_entry_point`）或各 RL 框架的配置文件。
>     
...
# 1. 解析命令行参数生成环境配置
env_cfg = parse_env_cfg(
    args_cli.task,                   # 要创建的环境 ID，比如 "Isaac-Cartpole-v0"
    device=args_cli.device,
    num_envs=args_cli.num_envs,
    use_fabric=not args_cli.disable_fabric
)

# 2. 调用 gym.make，动态导入并实例化已经注册的环境
env = gym.make(args_cli.task, cfg=env_cfg)
```

- `gym.make(id, cfg=…)` 会：
    
    1. 在全局 registry 中查找对应 `id`；
        
    2. 根据注册时的 `entry_point` 导入环境类；
        
    3. 把 `cfg` 和其他 `kwargs` 传入构造函数，返回一个完整的环境实例。 
...
1. **统一接口，无需显式导入环境类**
    ...
    - 用户只需记住环境的 `id` 字符串，即可通过 `gym.make(id)` 创建环境，提升可移植性与可读性。
        
2. **集中管理，易于扩展**
    
    - 所有环境注册逻辑集中在子模块中，增加新环境只需在对应包里添加一段 `gym.register()` 调用，而不必修改用户脚本。
        
3. **内置默认配置**
    
    - `kwargs` 中的 `env_cfg_entry_point`、`rl_games_cfg_entry_point` 等，允许给环境提供一套开箱即用的默认参数，简化实验设置。
        
4. **兼容各种 RL 框架**
    
    - 注册时可传入多个 RL 库的配置入口（RL-Games、SB3、skrl、RSL-RL 等），后续使用同一套 `gym.make` API 即可灵活切换框架。
        
5. **支持向量化环境**
    
    - 注册的 Manager-Based 环境（如 `ManagerBasedRLEnv`）实现了并行子环境管理，用户只需在配置里指定 `num_envs`，即可自动获得批量步进与回报计算能力。
        
6. **避免硬编码**
    
    - 通过字符串注册与构造，环境的实现与调用解耦，用户脚本中无须硬编码具体类路径，便于版本迭代和重构。
        

---

通过以上模式，Isaac Lab 利用了 Gym 的扩展机制，将复杂的物理仿真环境与标准化的 RL 接口（`reset`、`step`、`render`、`close`）无缝融合，极大地简化了研发和实验流程。
...
import sys       # 用于操作解释器参数列表

from isaaclab.app import AppLauncher  # 从 isaaclab 引入 App 启动器

# 添加 argparse 参数
parser = argparse.ArgumentParser(description="Train an RL agent with Stable-Baselines3.")  # 创建参数解析器
parser.add_argument("--video", action="store_true", default=False, help="Record videos during training.")  # 是否录制视频
parser.add_argument("--video_length", type=int, default=200, help="Length of the recorded video (in steps).")  # 视频长度
parser.add_argument("--video_interval", type=int, default=2000, help="Interval between video recordings (in steps).")  # 视频录制间隔
parser.add_argument("--num_envs", type=int, default=None, help="Number of environments to simulate.")  # 环境数量
parser.add_argument("--task", type=str, default=None, help="Name of the task.")  # 任务名称
...
# 为 Hydra 清理 sys.argv，只保留 hydra 参数
...
from stable_baselines3 import PPO  # 算法接口
from stable_baselines3.common.callbacks import CheckpointCallback  # 检查点回调
from stable_baselines3.common.logger import configure  # 日志配置
from stable_baselines3.common.vec_env import VecNormalize  # 向量化环境归一化

from isaaclab.envs import (
    DirectMARLEnv,           #直接多智能体（Multi-Agent RL）环境
    DirectMARLEnvCfg,        
    DirectRLEnvCfg,          #单智能体（Direct RL）环境的配置
    ManagerBasedRLEnvCfg,    #基于管理器的环境配置
    multi_agent_to_single_agent, #多智能体转单智能体接口函数
)
from isaaclab.utils.dict import print_dict  # 打印字典工具
from isaaclab.utils.io import dump_pickle, dump_yaml  # 配置保存

from isaaclab_rl.sb3 import Sb3VecEnvWrapper, process_sb3_cfg  # 封装与配置处理

import isaaclab_tasks  # noqa: F401  # 任务包
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # 用命令行 args 覆盖 Hydra 配置
    env_cfg.scene.num_envs = args_cli.num_envs if args_cli.num_envs is not None else env_cfg.scene.num_envs  # 环境数量
   
	agent_cfg["seed"] = args_cli.seed if args_cli.seed is not None else agent_cfg["seed"]  # 训练种子
    ...
    env_cfg.sim.device = args_cli.device if args_cli.device is not None else env_cfg.sim.device  # 指定设备

    # 日志目录设置
    run_info = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")  # 当前时间字符串
    log_root_path = os.path.abspath(os.path.join("logs", "sb3", args_cli.task))  # 日志根目录
    print(f"[INFO] Logging experiment in directory: {log_root_path}")
    # Ray Tune 依赖此输出解析实验名称，勿改
    print(f"Exact experiment name requested from command line: {run_info}")
    log_dir = os.path.join(log_root_path, run_info)  # 本次实验日志目录
    # 保存配置到 YAML 与 Pickle
    ...
    if args_cli.video:
        video_kwargs = {
            ...
            "video_length": args_cli.video_length,  # 录制长度
            "disable_logger": True,
        }
...
代码中绝大部分都是写日志和rl库的样板代码，重要的是下面的三个包装器。
#### 1. 多智能体→单智能体 转换
...
- **作用**：如果环境是多智能体（`DirectMARLEnv`），则调用 `multi_agent_to_single_agent`，把它封装成一个单智能体接口，便于用 SB3 这类只支持单智能体的算法进行训练。
    
- **原理**：内部通常会把每个智能体的观测拼接成一个大观测，将各智能体动作合并成一个动作向量，所有 step/reward/done 都对应处理。
    

---

#### 2. 录制视频的包装（RecordVideo）
...
- **作用**：给原始环境套上一层视频录制包装。每当 `step_trigger(step)` 返回 `True` 时，就开始录制后续 `video_length` 步（帧）的环境交互，并以视频文件形式保存在 `video_folder`。
    
- **关键参数**：
    
    - `video_folder`：存放视频的目录
        
    - `step_trigger`：一个函数，接受当前全局步数，决定是否触发新一段录制
        
    - `video_length`：触发后录制的最大帧数
        
    - `disable_logger`：关闭内部 Gym 日志，以免过多输出
        

---

#### 3. 自定义 SB3 向量化环境包装（Sb3VecEnvWrapper）
...
- **作用**：把单个 Gym 环境封装成一个**向量化环境**（vectorized environment），用于并行执行多实例，提供与 Stable-Baselines3 接口兼容的 `reset()`、`step()` 等方法。
    
- **为什么要用**：SB3 的许多算法（包括 PPO）在内部默认使用 `VecEnv` 接口，以便一次性对多个环境副本并行进行采样，提升效率。
    
- **内部可能做的事**：
    
    1. 将单个 env 克隆成 `num_envs` 份
        
    2. 并行管理它们的 `step()` 调用
        
    3. 返回形如 `(obs_batch, rewards_batch, dones_batch, infos_batch)`
        

---

#### 4. 归一化包装（VecNormalize）
...
- **作用**：对向量化环境的观测（obs）和奖励（reward）进行在线统计与归一化，常用于加速训练和稳定学习过程。
    
- **主要参数**：
    
    - `norm_obs`：是否对观测做均值方差归一化
        
    - `norm_reward`：是否对奖励做归一化
        
    - `clip_obs`：对归一化后观测的裁剪阈值
        
    - `gamma`：用于奖励回报折扣的γ，影响 reward 归一化的统计
        
    - `training`：设为 `True` 时，包装器在调用 `step()` 时更新统计量
